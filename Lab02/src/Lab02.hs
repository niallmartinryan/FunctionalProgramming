...
getMeasured rqdDist [] = []
getMeasured rqdDist (x:ps)
      | rqdDist > distance x = [x] ++ getMeasured (rqdDist - distance x) ps
...
findFastest rqdDist p = findFastestOne rqdDist p 0 []


findFastestOne :: Distance
                  -> SegPath
                  -> Double
                  -> SegPath
                  -> Maybe
                    ( SegPath
                      ,Speed)
-- findFastestOne rqdDist p 0 y = findFastestOne rqdDist p (rqdDist/time (getMeasured rqdDist (p))) y 
findFastestOne rqdDist [] curFast y | y ==[] = Nothing
                                    | otherwise = Just (y,curFast)
findFastestOne rqdDist x curFast []  | pathDistance x < rqdDist = Nothing
findFastestOne rqdDist (x:[]) curFast y | pathSpeed fire > curFast && pathDistance [x] >= rqdDist  = findFastestOne rqdDist [] (pathSpeed fire) fire
                                        | otherwise = findFastestOne rqdDist [] curFast y
                                        where fire = getMeasured rqdDist ([x])
findFastestOne rqdDist (x:ps) curFast y | pathSpeed fire > curFast && pathDistance (x:ps) >= rqdDist = findFastestOne rqdDist ps (pathSpeed fire) fire
                                        | otherwise = findFastestOne rqdDist ps curFast y 
                                        where fire = getMeasured rqdDist (x:ps)
