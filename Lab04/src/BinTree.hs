...
treeShowDatum :: (Show k, Show d) => Bool -> BinTree k d -> String
treeShowDatum _ Empty = "Empty"
treeShowDatum inDeep (Leaf key datum)
  = parenthesiseIf2 inDeep (show key++" "++show datum)
treeShowDatum inDeep (Branch ltree key datum rtree)
 = parenthesiseIf2 inDeep
     (treeShowDatum True ltree++","++show key++" "++show datum++","++treeShowDatum True rtree)

parenthesiseIf2 inDeep str
 | inDeep     =  	  str
 | otherwise  =       str
